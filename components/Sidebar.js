...
export default function Sidebar() {
  ...
  const chatExists = email => chat?.find(chat => (chat.users.includes(user.email) && chat.users.includes(email)))

  const newChat = async () => {
    const input = prompt("Digite o email do destinatário do chat");
    if (!chatExists(input) && (input != user.email)) {
      await addDoc(collection(db, "chat"), { users: [user.email, input] })
    }
  }

  const chatList = () => {
    return (
      chat?.filter(chat => chat.users.includes(user.email))
      .map(
        chat => 
  ...
  return (
    ...
    >
      ...
      <Button m={5} p={4} onClick={() => newChat()}>Novo Chat</Button>
